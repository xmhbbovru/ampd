...
UseIndexAndDevReturn use_index_and_dev(Vad v, bool col_zero) {
  ...
  if (col_zero) {
    ...
    return make_pair(nr_nonzero == 0,
                     double(nr_nonzero) / double(v.size()));
  } else {
    double sum_sq_dev = calc_sum_sq_dev(v);
    return make_pair(sum_sq_dev < sum_sq_dev_min,
                     sqrt(sum_sq_dev / double(v.size())));
  }
}

AmpdReturn ampd(Vad x, double alg_mean, double alg_stdev,
                    ...
                    const char *peaks_str=NULL, const char *troughs_str=NULL)
{
  ...
  for (int i = 0; i < n; i++) {
    ...
    UseIndexAndDevReturn pk_ret = use_index_and_dev(Vad(mpk[slice(i,
                                                                  pk_lamb + 1,
                                                                  n)]),
                                                    col_zero);
    pk_dev[i] = pk_ret.second; // for writing files for graphs
    // could use use_index but want to use same code for pk and tr
    UseIndexAndDevReturn tr_ret = use_index_and_dev(Vad(mtr[slice(i,
                                                                  tr_lamb + 1,
                                                                  n)]),
                                                    col_zero);    
  ...
  return make_tuple(el, pk_lamb, tr_lamb,
                    Vad(pk_zero_dev_ixs[slice(0, pk_zero_ixs_ix, 1)]),
                    Vad(tr_zero_dev_ixs[slice(0, tr_zero_ixs_ix, 1)]));
}

Vad test_data(double a,  double b,  double c,  double d,
...
int main(int argc, char *argv[]) {
  ...
  Vad v = test_data(a, b, c, d, f1, f2, f3_start, f3_end, fs, start_t, len_t,
  ...
  Vad peaks = get<3>(calc);   // unused below
  Vad troughs = get<4>(calc); // unused below
